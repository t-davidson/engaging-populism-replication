...
m_populist <- readRDS("../output/tw_retweets_populist_cat_rs_cy.rds")
...
results <- results %>% left_join(tw_activity, by = c("country", "year"))
...
p <- results %>% ggplot(aes(y = effect, x = year, ymin = .lower, ymax = .upper, group = term, color = Populism)) +
    geom_line() +
    geom_pointinterval(aes(shape = ol), position = position_dodge(0.8), interval_size_range = c(0.5,0.8), point_size = 1)  +
...
p
ggsave2(filename = "figures/figure6_retweets.pdf", width = 8.3, height = 9.7)
