...
y <- ggplot(slopes.y, aes(x = effect, y = as.factor(year), fill = after_stat(abs(x) > 1))) +
    ...
    labs(y="", x = TeX("$exp(\\hat{\\beta_1}_{Populism} + \\hat{\\gamma_1}_{Year})$")) + xlim(0, 3)
y

ggarrange(c, y, labels = c("A", "B"))
ggsave2(filename = "figures/country_year_slopes_tw_populist_likes.pdf", width = 8.3, height = 10)
...
ggsave2(filename = "figures/country_slopes_tw_likes.pdf", width = 8.3, height = 9)
...
ggsave2(filename = "figures/year_slopes_tw_likes.pdf", width = 8.3, height = 9)
