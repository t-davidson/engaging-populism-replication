...
m <- brm(
    total_retweets ~ log(total_tweets) +
      poppa_populism_s +
      # ideology
      left_right_s +
      # party
      ...
      election_year +
      # random part
      (1 |twitter_handle) + (1 + poppa_populism_s |country) + (1 + poppa_populism_s |year.f) + (1 + poppa_populism_s |country_year),
    ...
    control = list(
        adapt_delta = 0.95),
    file = 'model_results/tw_retweets_poppa_rs_cy',
    seed = seed,
    backend = "cmdstanr")
