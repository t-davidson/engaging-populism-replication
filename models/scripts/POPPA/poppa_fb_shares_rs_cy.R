...
m <- brm(
      total_shares ~ log(total_posts) +
        ...
        election_year +
        # random part
        (1 |handle) + (1 + poppa_populism_s |country) + (1 + poppa_populism_s |year.f) + (1 + poppa_populism_s |country_year),
    ...
    control = list(
        adapt_delta = 0.95),
    file = 'model_results/fb_shares_poppa_rs_cy',
    seed = seed,
    backend = "cmdstanr")
